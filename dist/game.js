...
function startGame(){puzzleIndex=(puzzleIndex+1)%PUZZLES.length;const puzzle=window.EXTRA_PUZZLE||window.DAILY_PUZZLE||PUZZLES[puzzleIndex];const[a,b,name]=puzzle;words=randomStartingWords(a,b);score=0;won=false;selected=null;activeClue=null;$('word').value='';$('word').disabled=false;document.querySelector('.submit').disabled=false;$('clue').disabled=false;clearClue();$('puzzle-name').textContent=name;$('puzzle-mode').textContent=window.DAILY_MODE?'DAILY CHALLENGE':window.EXTRA_LEVEL?'EXTRA LEVEL '+window.EXTRA_LEVEL:'EXTRA PUZZLE';$('start-one').textContent=a;$('start-two').textContent=b;feedback('Connect the two green words to finish.');setDirection('H');}
