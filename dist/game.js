...
function startGame(){puzzleIndex=(puzzleIndex+1)%PUZZLES.length;const puzzle=window.DAILY_PUZZLE||PUZZLES[puzzleIndex];const[a,b,name]=puzzle;words=[{text:a,r:3,c:3,dir:'H'},{text:b,r:11,c:7,dir:'H'}];score=0;won=false;selected=null;$('word').value='';$('word').disabled=false;document.querySelector('.submit').disabled=false;$('clue').disabled=false;clearClue();$('puzzle-name').textContent=name;$('puzzle-mode').textContent=window.DAILY_MODE?'DAILY CHALLENGE':'EXTRA PUZZLE';$('start-one').textContent=a;$('start-two').textContent=b;feedback('Connect the two green words to finish.');setDirection('H');}
