...
let words=[], selected=null, direction='H', score=0, won=false, puzzleIndex=-1, activeClue=null;
...
function submitWord(text,r,c,dir){if(!Number.isInteger(r)||!Number.isInteger(c)||!['H','V'].includes(dir))return {error:'Choose a starting square and direction.'};text=String(text).trim().toUpperCase();const result=validate(text,r,c,dir);if(result.error){feedback(result.error,'error');return result;}words.push({text,r,c,dir});score+=result.cost;won=result.bridge;$('word').value='';clearClue();feedback(won?`Connected! You joined ${words[0].text} and ${words[1].text} in ${score} points. ${window.DAILY_MODE?'Your daily result is saved below.':'Try another extra puzzle when you are ready.'}`:`${text} linked. +${result.cost} points. Keep the connection going.`, 'success');$('word').disabled=won;document.querySelector('.submit').disabled=won;$('clue').disabled=won;render();if(won&&typeof window.wordLinksCompleted==='function')window.wordLinksCompleted({score,words:words.map(w=>w.text)});return {word:text,score,complete:won};}
function randomStartingWords(a,b){for(let attempt=0;attempt<200;attempt++){const first={text:a,r:Math.floor(Math.random()*15),c:Math.floor(Math.random()*(16-a.length)),dir:'H'};const second={text:b,r:Math.floor(Math.random()*15),c:Math.floor(Math.random()*(16-b.length)),dir:'H'};const firstCells=cells(first),secondCells=cells(second);if(first.r===second.r&&firstCells.some(x=>secondCells.some(y=>x.c===y.c)))continue;if(firstCells.some(x=>secondCells.some(y=>Math.abs(x.r-y.r)<=1&&Math.abs(x.c-y.c)<=1)))continue;return [first,second];}return [{text:a,r:1,c:1,dir:'H'},{text:b,r:12,c:7,dir:'H'}];}
function startGame(){puzzleIndex=(puzzleIndex+1)%PUZZLES.length;const puzzle=window.DAILY_PUZZLE||PUZZLES[puzzleIndex];const[a,b,name]=puzzle;words=randomStartingWords(a,b);score=0;won=false;selected=null;activeClue=null;$('word').value='';$('word').disabled=false;document.querySelector('.submit').disabled=false;$('clue').disabled=false;clearClue();$('puzzle-name').textContent=name;$('puzzle-mode').textContent=window.DAILY_MODE?'DAILY CHALLENGE':'EXTRA PUZZLE';$('start-one').textContent=a;$('start-two').textContent=b;feedback('Connect the two green words to finish.');setDirection('H');}
